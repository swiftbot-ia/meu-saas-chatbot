    ...
    try {
      ...
      if (this.isWhatsAppUrl(mediaUrl) && instanceToken) {
        ...
        try {
          ...
          // Para áudios, UazAPI retorna MP3 automaticamente (generate_mp3: true)
          buffer = await uazapi.downloadMedia(instanceToken, messageId, messageType)

          // Determinar mimetype pelo tipo de mensagem
          // IMPORTANTE: Para áudios, UazAPI retorna MP3 (não OGG)
          const mimeTypeMap = {
            'audio': 'audio/mpeg',  // ← MP3 (UazAPI converte automaticamente)
            'image': 'image/jpeg',
            'video': 'video/mp4',
