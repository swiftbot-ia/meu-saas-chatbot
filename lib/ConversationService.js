...
class ConversationService {
   ...
   */
  static async getOrCreateConversation(instanceName, connectionId, contactId, userId) {
    const chatSupabase = createChatSupabaseClient();
   ...
   */
  static async listConversations(userId, { connectionId, search, limit = 50, offset = 0, includeArchived = false } = {}) {
    const mainSupabase = createServerSupabaseClient();
    const chatSupabase = createChatSupabaseClient();

    console.log('[ConversationService] listConversations called with:', { userId, connectionId, search, limit, offset, includeArchived });
    ...
    if (connectionId) {
      ...
      if (connError) {
        console.error('[ConversationService] Error fetching connection:', connError);
      }

      if (connection) {
        instanceName = connection.instance_name;
        console.log('[ConversationService] Using instance_name:', instanceName);
      }
    }
    ...
    // Filter archived by default (only show active conversations)
    if (!includeArchived) {
    ...
    if (search) {
      ...
      const { data: conversations, error: searchError } = await query;

      if (searchError) {
        console.error('[ConversationService] Error searching conversations:', searchError);
        throw searchError;
      }

      console.log('[ConversationService] Search found conversations:', conversations?.length || 0);
      ...
      if (filtered && filtered.length > 0) {
        const instanceNames = [...new Set(filtered.map(c => c.instance_name))];
    ...
    console.log('[ConversationService] Found conversations:', data?.length || 0, 'total:', count);
