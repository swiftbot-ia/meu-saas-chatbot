...
class ConversationService {
  ...
  static async listConversations(userId, { connectionId, search, limit = 50, offset = 0 } = {}) {
    ...
    // Filter by connection_id directly if specified
    if (connectionId) {
      console.log('🔍 [ConversationService] Filtering by connection_id:', connectionId);
      query = query.eq('connection_id', connectionId);
    } else {
      console.log('⚠️ [ConversationService] No connection filter applied');
    }
    ...
    if (data && data.length > 0) {
      const connectionIds = [...new Set(data.map(c => c.connection_id))];
      ...
      data.forEach(conv => {
        conv.connection = connections?.find(c => c.id === conv.connection_id);
      });
    }
