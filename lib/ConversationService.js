...
class ConversationService {
  ...
  static async listConversations(userId, { connectionId, search, limit = 50, offset = 0 } = {}) {
    ...
    console.log(`🔍 [ListConversations] connectionId=${connectionId}, instanceName=${instanceName}`);
    ...
    if (instanceName) {
      console.log(`🔍 [ListConversations] Applying filter: instance_name = ${instanceName}`);
      query = query.eq('instance_name', instanceName);
    } else {
      console.log(`⚠️ [ListConversations] NO instance_name filter applied!`);
    }
    ...
    if (data && data.length > 0) {
      ...
      // DEBUG: Log unique instance_names in results
      console.log(`🔍 [ListConversations] Results contain ${data.length} conversations from instance_names:`, instanceNames);
