...
class ConversationService {
  ...
  static async listConversations(userId, { connectionId, search, limit = 50, offset = 0 } = {}) {
    ...
    console.log('📋 [ConversationService] Listing conversations:', { userId, connectionId });

    // Try to create chat client and catch configuration errors
    let chatSupabase;
    try {
      chatSupabase = createChatSupabaseClient();
    } catch (error) {
      console.error('❌ [ConversationService] Erro ao criar cliente do banco de chat:', error.message);
      throw new Error(
        'Banco de dados de chat não configurado. ' +
        'Verifique as variáveis NEXT_PUBLIC_CHAT_SUPABASE_URL e NEXT_PUBLIC_CHAT_SUPABASE_ANON_KEY'
      );
    }
    ...
    if (connectionId) {
      ...
      if (connection) {
        ...
        console.log('✅ [ConversationService] Found instance_name:', instanceName);
      } else {
        console.warn('⚠️ [ConversationService] No connection found for ID:', connectionId);
      }
    }
    ...
    if (instanceName) {
      console.log('🔍 [ConversationService] Filtering by instance_name:', instanceName);
      query = query.eq('instance_name', instanceName);
    } else {
      console.log('⚠️ [ConversationService] No instance filter applied');
    }
