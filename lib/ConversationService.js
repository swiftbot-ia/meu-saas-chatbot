...
class ConversationService {
   ...
   */
  static async listConversations(userId, { connectionId, search, limit = 50, offset = 0, currentUserId } = {}) {
    ...
    // Check permissions filter if currentUserId is provided
    let permissionFilter = null;
    if (currentUserId) {
      try {
        const account = await getAccountForUser(currentUserId);
        // If user is a consultant, filter conversations.
        // Note: getAccountForUser returns role in 'userRole' property
        console.log(`🔍 [ListConv] Checking role for user ${currentUserId}: ${account?.userRole}`);
        if (account && account.userRole === 'consultant') {
          permissionFilter = `assigned_to.is.null,assigned_to.eq.${currentUserId}`;
          console.log('🔒 [ListConv] Consultant filter applied:', permissionFilter);
        }
      } catch (err) {
        console.error('Error checking permissions for listConversations:', err);
      }
    }
    ...
    // Apply permission filter for consultants
    if (permissionFilter) {
      query = query.or(permissionFilter);
    }
   ...
   */
  static async updateConversation(conversationId, updates, customChatClient = null) {
    const chatSupabase = customChatClient || createChatSupabaseClient();
   ...
   */
  static async assignConversation(conversationId, assigneeId) {
    const chatSupabase = createChatSupabaseClient();
