...
class AutomationService {
    ...
    static async processMessage(message, connection, contact) {
        try {
            ...
            const { data: automations, error } = await getSupabaseAdmin()
                .from('automations')
                .select(`
          *,
    ...
    static async incrementExecutionCount(automationId) {
        try {
            ...
            const { data: automation } = await getSupabaseAdmin()
                .from('automations')
    ...
    static async executeActions(automation, connection, contact, message) {
        try {
            ...
            if (automation.action_add_tags && automation.action_add_tags.length > 0) {
                ...
                for (const tagName of automation.action_add_tags) {
                    // Get tag ID
                    const chatSupabaseAdmin = (await import('@/lib/supabase/chat-server')).chatSupabaseAdmin
                    const { data: tag } = await chatSupabaseAdmin
                        .from('contact_tags')
                        .select('id')
                    ...
                    if (tag) {
                        SequenceService.checkAndEnroll(contact.id, connection.id, 'has_tag', tag.id)
