...
class AutomationService {
    ...
    static async executeActions(automation, connection, contact, message) {
        try {
            // Import SequenceService dynamically to avoid circular dependency
            const SequenceService = (await import('@/lib/SequenceService')).default

            // 1. Definir origem se configurado (Antes do webhook para que vá atualizado)
            if (automation.action_set_origin_id) {
                await this.setContactOrigin(contact.id, automation.action_set_origin_id)

                // Atualizar objeto contact localmente para o webhook
                contact.origin_id = automation.action_set_origin_id

                // Check for sequences triggered by this origin
                SequenceService.checkAndEnroll(contact.id, connection.id, 'has_origin', automation.action_set_origin_id)
                    .catch(err => console.error('❌ [Automation] Erro ao verificar sequências has_origin:', err))
            }

            // 2. Adicionar tags ao contato se configurado
            if (automation.action_add_tags && automation.action_add_tags.length > 0) {
                // action_add_tags contém IDs de tags
                await this.addTagIdsToContact(contact.id, automation.action_add_tags)

                // Check for sequences triggered by these tags
                for (const tagId of automation.action_add_tags) {
                    // Check triggers using ID directly since we have it
                    SequenceService.checkAndEnroll(contact.id, connection.id, 'has_tag', tagId)
                        .catch(err => console.error('❌ [Automation] Erro ao verificar sequências has_tag:', err))
                }
            }

            // 3. Enviar para webhook externo se configurado (POR ÚLTIMO, com dados atualizados)
    ...
    /**
     * Adiciona tags existentes (por ID) ao contato
     */
    static async addTagIdsToContact(contactId, tagIds) {
        const chatSupabaseAdmin = (await import('@/lib/supabase/chat-server')).chatSupabaseAdmin

        try {
            for (const tagId of tagIds) {
                // 1. Verificar se já tem assignment
                ...
                if (!existingAssignment) {
                    ...
                    if (assignError) {
                        console.error(`❌ [Automation] Erro ao atribuir tag ID "${tagId}":`, assignError)
                    } else {
                        console.log(`🏷️ [Automation] Tag ID "${tagId}" atribuída ao contato`)
                    }
                }
            }
        } catch (error) {
            console.error('❌ [Automation] Erro ao adicionar tags (IDs):', error)
        }
    }

    /**
     * Adiciona tags ao contato usando nomes (cria se necessário)
     * Mantido para compatibilidade com legados
     */
