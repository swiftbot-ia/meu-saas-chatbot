...
const SequenceService = {
    ...
    async enrollContact(sequenceId, contactId, connectionId) {
        ...
        try {
            ...
            // Get conversation_id for this contact/connection
            ...
            await supabase.rpc('increment_subscribers_count', { seq_id: sequenceId })
                .catch(() => {
    ...
    async processPendingSubscriptions() {
        ...
        try {
            // Get all subscriptions due for next step
     ...
     * @param {object} subscription - Subscription with sequence and steps
     */
    async processSubscription(subscription) {
        ...
        if (!currentStep.is_active) {
            // Skip to next step
            return this.advanceToNextStep(subscription, steps, currentStepIndex)
        }
        ...
        if (!contact) {
            console.error(`❌ [SequenceService] Contact not found: ${subscription.contact_id}`)
            return
        }

        // Get connection details for sending
        ...
        // Get message content (from template or step)
        ...
        await this.advanceToNextStep(subscription, steps, currentStepIndex)
    },

    /**
     * Advance subscription to next step
     */
    async advanceToNextStep(subscription, steps, currentStepIndex) {
        const supabase = getServiceClient()
