...
class SyncService {
    ...
    static async executeFullSync(jobId, connectionId, instanceToken) {
        try {
            ...
            // 4. Marcar has_synced = true no banco
            await supabaseAdmin
                .from('whatsapp_connections')
                .update({ has_synced: true })
                .eq('id', connectionId)

            log(jobId, '💾', 'has_synced marcado como true')

            // 5. Disparar análise de conversas com IA (em background)
            log(jobId, '🤖', 'Iniciando análise de conversas com IA...')
            try {
                const { ConversationAnalyzer } = await import('./conversation-analysis/analyzer.js')
                const analyzer = new ConversationAnalyzer()
                const analysisJob = await analyzer.startAnalysis(connectionId, connection.user_id, {})
                log(jobId, '📊', `Análise iniciada: ${analysisJob.id}`)
            } catch (analysisError) {
                // Log error but don't fail the sync
                log(jobId, '⚠️', 'Erro ao iniciar análise (sync continua)', { error: analysisError.message })
            }
            ...
            for (let i = 0; i < individualContacts.length; i += CONTACTS_BATCH_SIZE) {
                ...
                for (const contact of batch) {
                    try {
                        ...
                        if (i === 0 && batch.indexOf(contact) === 0) {
                            log(jobId, '🔍', 'Exemplo de contato UAZAPI:', contact)
                        }

                        // Extrair nome com múltiplos fallbacks (UAZAPI usa contact_name)
                        ...
                        // Extrair foto de perfil com múltiplos fallbacks
                        const profilePicUrl = contact.imgUrl ||
                            ...
                            contact.image ||
                            contact.picture ||
                            null            // Criar/atualizar contato usando RPC existente
                        const result = await ConversationService.getOrCreateContact(
                            whatsappNumber,
                            { name, jid, profilePicUrl },
                            chatSupabaseAdmin
                        )

                        // FORÇAR atualização se temos nome/foto melhor
                        if (name && name !== whatsappNumber) {
                            await ConversationService.updateContactInfo(
                                result.id,
                                { name, profilePicUrl },
                                chatSupabaseAdmin
                            )
            ...
            for (let i = 0; i < conversations.length; i += CONVERSATIONS_BATCH_SIZE) {
                ...
                for (const conv of batch) {
                    ...
                    try {
                        ...
                        // Debug: mostrar chatId usado na primeira busca
                        if (i === 0 && batch.indexOf(conv) === 0) {
                            log(jobId, '🔍', `DEBUG chatId: "${chatId}" | número: "${conv.contact.whatsapp_number}"`)
                        }

                        log(jobId, '📨', `Buscando mensagens: ${conv.contact.name || conv.contact.whatsapp_number}`)

                        const messages = await uazClient.getMessages(instanceToken, chatId)

                        // Debug: log first message set structure
                        if (i === 0 && batch.indexOf(conv) === 0 && messages) {
                            log(jobId, '🔍', 'Estrutura de resposta mensagens:', {
                                isArray: Array.isArray(messages),
                                length: Array.isArray(messages) ? messages.length : 'N/A',
                                type: typeof messages,
                                sample: Array.isArray(messages) && messages[0] ? JSON.stringify(messages[0]).substring(0, 300) : 'empty'
                            })
                        }

                        if (Array.isArray(messages) && messages.length > 0) {
                            log(jobId, '💾', `Salvando ${messages.length} mensagens...`)
                            ...
                            log(jobId, '✅', `Salvas: ${savedCount}/${messages.length}`)
                        } else {
                            log(jobId, '⚠️', `Sem mensagens para ${conv.contact.name || conv.contact.whatsapp_number}`)
                        }
     ...
     * Para áudios: transcreve com OpenAI
     * Para outros tipos: salva sem processamento adicional
     */
    static async saveMessages(jobId, conversationId, contactId, connection, messages, instanceToken) {
        let savedCount = 0
        const uazClient = new UAZAPIClient()

        for (const msg of messages) {
            try {
                // Extrair ID da mensagem (múltiplos formatos possíveis)
                ...
                if (!messageId) {
                    log(jobId, '⚠️', 'Mensagem sem ID, pulando', { keys: Object.keys(msg) })
                    continue
                }
                ...
                if (msgType.toLowerCase().includes('text') || msgType === 'Conversation' || msgType === 'conversation') {
                    messageType = 'text'
                    messageContent = content.text || msg.text || content.conversation || ''
                } else if (msgType.toLowerCase().includes('extended')) {
                    messageType = 'text'
                    messageContent = content.text || msg.text || ''
                } else if (msgType.toLowerCase().includes('image')) {
                    messageType = 'image'
                    messageContent = content.caption || msg.caption || ''
                    mediaUrl = content.URL || content.url || msg.mediaURL || null
                    ...
                    // Transcrever áudio com OpenAI (async, não bloqueia)
                    if (mediaUrl) {
                        try {
                            log(jobId, '🎤', `Transcrevendo áudio: ${messageId}`)
                            const { default: MediaServiceVPS } = await import('./MediaServiceVPS')
                            const audioResult = await MediaServiceVPS.processAudio(
                                msg,
                                instanceToken,
                                null // Não salvar no disco durante sync
                            )
                            if (audioResult?.transcription) {
                                transcription = audioResult.transcription
                                messageContent = transcription
                                log(jobId, '✅', `Áudio transcrito: "${transcription.substring(0, 50)}..."`)
                            }
                        } catch (audioError) {
                            log(jobId, '⚠️', `Erro ao transcrever áudio: ${audioError.message}`)
                            // Continua sem transcrição
                        }
                    }
                } else if (msgType.toLowerCase().includes('document')) {
                    messageType = 'document'
                    messageContent = content.fileName || content.title || ''
                    mediaUrl = content.URL || content.url || msg.mediaURL || null
                } else if (msgType.toLowerCase().includes('sticker')) {
                    messageType = 'sticker'
                    mediaUrl = content.URL || content.url || msg.mediaURL || null
                } else if (msgType.toLowerCase().includes('location')) {
                    messageType = 'location'
                    messageContent = `${content.degreesLatitude || 0}, ${content.degreesLongitude || 0}`
                } else if (msgType.toLowerCase().includes('contact')) {
                    messageType = 'contact'
                    messageContent = content.displayName || ''
                }
                ...
                // Adicionar transcrição se existir
                if (transcription) {
                    insertData.transcription = transcription
                    insertData.transcription_status = 'completed'
                }
                ...
                if (insertError) {
                    log(jobId, '❌', `Erro ao inserir mensagem ${messageId}`, {
