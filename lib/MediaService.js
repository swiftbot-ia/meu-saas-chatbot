...
class MediaService {
  ...
  async decryptWhatsAppMedia(encryptedBuffer, keys) {
    try {
      ...
      // Expand media key using HKDF (simplified version for WhatsApp media)
      // WhatsApp uses first 32 bytes for IV, next 32 for cipher key
      const mediaKeyExpandedFull = this.hkdfExpand(mediaKeyExpanded, 112);
