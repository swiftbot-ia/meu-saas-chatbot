...
class MediaService {
  ...
  async decryptWhatsAppMedia(encryptedBuffer, keys) {
    try {
      // Decode base64 keys
      const mediaKeyExpanded = Buffer.from(keys.mediaKey, 'base64');
      ...
      // Expand media key using HKDF with WhatsApp's info parameter
      // Info format: "WhatsApp {Media Type} Keys" (e.g., "WhatsApp Audio Keys")
      const mediaType = this.getMediaTypeFromMimeType(keys.mimetype || 'audio/ogg');
      const info = Buffer.from(`WhatsApp ${mediaType} Keys`);

      const mediaKeyExpandedFull = this.hkdfExpandWhatsApp(mediaKeyExpanded, info, 112);
      ...
      console.log(`🔑 Using info: "WhatsApp ${mediaType} Keys"`);
   ...
   * @param {Buffer} key - Media key (32 bytes)
   ...
   */
  hkdfExpandWhatsApp(key, info, length) {
