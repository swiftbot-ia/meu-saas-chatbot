 ...
 * Supports WhatsApp encrypted media decryption
 */

import fs from 'fs/promises';
import path from 'path';
import crypto from 'crypto';
import whatsappMediaDecryptor from './WhatsAppMediaDecryptor.js';
...
class MediaService {
   ...
   * Automatically handles WhatsApp encrypted media decryption
   ...
   * @param {string} mediaKey - Base64-encoded WhatsApp media encryption key (for encrypted media)
   ...
   */
  async downloadAndSaveMedia(mediaUrl, mediaType, messageId, knownMimeType = null, mediaKey = null, fileSHA256 = null) {
    try {
      console.log(`📥 Downloading media: ${mediaType} from ${mediaUrl.substring(0, 80)}...`);

      let buffer;
      let isEncrypted = false;

      // Check if this is encrypted WhatsApp media
      if (mediaKey && whatsappMediaDecryptor.isEncryptedUrl(mediaUrl)) {
        isEncrypted = true;
        console.log('🔐 Detected encrypted WhatsApp media - will decrypt');

        // Download and decrypt WhatsApp media
        buffer = await whatsappMediaDecryptor.downloadAndDecrypt(
          mediaUrl,
          mediaKey,
          mediaType,
          fileSHA256
        ...
        console.log(`✅ Decrypted WhatsApp media successfully (${buffer.length} bytes)`);
      } else {
        // Regular download for non-encrypted media
      ...
      // Determine MIME type (priority: knownMimeType > detection > default)
      let mimeType = knownMimeType;
      if (!mimeType || mimeType === 'application/octet-stream') {
        mimeType = this.getMimeType(mediaType); // Fallback to default
      }

      console.log(`📄 Media MIME type: ${mimeType} (encrypted: ${isEncrypted})`);
      ...
      // Save decrypted/downloaded file
