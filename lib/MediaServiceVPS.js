...
class MediaServiceVPS {
    ...
    static async downloadDocumentFromUAZapi(messageId, token, metadata = {}) {
        try {
            ...
            if (!fileName) {
                fileName = this.generateFileName(messageId, extension);
            } else {
                // Sanitizar nome vindo do WhatsApp
                fileName = this.sanitizeFileName(fileName);
            }

            // Se o nome sanitizado não tiver extensão, adicionar
    ...
    static sanitizeFileName(fileName) {
        ...
        // Remove caracteres não seguros (mantém letras, números, pontos, traços e underscores)
        sanitized = sanitized.replace(/[^a-zA-Z0-9._-]/g, '');

        // Limita o tamanho (255 é o padrão em muitos sistemas, mas vamos ser conservadores)
    ...
    static async downloadAndSave(mediaUrl, mediaType, messageId, metadata = {}) {
        try {
            ...
            let fileName = metadata.fileName || this.generateFileName(messageId, extension);

            // SANITIZAR NOME DO ARQUIVO
            if (metadata.fileName) {
                fileName = this.sanitizeFileName(fileName);
                console.log(`🧹 [MediaVPS] Sanitized filename: ${metadata.fileName} -> ${fileName}`);
            }
