...
class MediaServiceVPS {
    ...
    static async processMedia(mediaUrl, mediaType, messageId, metadata = {}) {
        try {
            ...
            if (mediaType === 'audio' && metadata.instanceToken) {
                try {
                    ...
                    // Verificar se a resposta é realmente um arquivo MP3 ou JSON de erro
                    const contentType = response.headers.get('content-type');
                    if (contentType && contentType.includes('application/json')) {
                        const errorData = await response.json();
                        throw new Error(`UAZapi returned JSON error: ${JSON.stringify(errorData)}`);
                    }

                    // Salvar MP3 diretamente
                    ...
                    // VALIDAR: MP3 deve ter pelo menos 1KB (caso contrário é JSON de erro)
                    if (stats.size < 1024) {
                        // Ler arquivo para ver se é JSON
                        const fileContent = fs.readFileSync(fullPath, 'utf8');
                        fs.unlinkSync(fullPath); // Deletar arquivo inválido

                        try {
                            const errorJson = JSON.parse(fileContent);
                            throw new Error(`UAZapi returned error: ${JSON.stringify(errorJson)}`);
                        } catch (e) {
