...
class MediaServiceVPS {
    ...
    /**
     * Baixa vídeo via UAZapi (descriptografa automaticamente)
     * 
     * @param {string} messageId - ID da mensagem
     * @param {string} token - Token da instância UAZapi
     * @returns {Promise<object>} { localPath, fullPath, mimeType, size, fileName }
     */
    static async downloadVideoFromUAZapi(messageId, token) {
        try {
            console.log(`🎥 [MediaVPS] Downloading video via UAZapi (decryption)...`);
            ...
            // Corrigir typo comum (htps → https)
            let videoUrl = data.fileURL.replace(/^htps:\/\//, 'https://');

            console.log(`📥 [MediaVPS] Downloading decrypted video from: ${videoUrl}`);

            // Baixar o vídeo descriptografado
            ...
            // Detectar extensão (preferir mp4 para vídeo)
            const contentType = videoResponse.headers.get('content-type') || 'video/mp4';
            let extension = this.getExtensionFromMimeType(contentType);
            if (extension === 'bin') extension = 'mp4';

            // Salvar vídeo
            const fileName = this.generateFileName(messageId, extension);
            const fullPath = path.join(process.cwd(), 'public', 'media', 'video', fileName);
            const localPath = `/media/video/${fileName}`;
            ...
            // VALIDAR: Vídeo deve ter pelo menos 1KB
            if (stats.size < 1024) {
                fs.unlinkSync(fullPath);
                throw new Error(`Downloaded video is too small (${stats.size} bytes) - likely corrupted`);
            }

            console.log(`✅ [MediaVPS] Video saved (decrypted): ${localPath} (${(stats.size / 1024).toFixed(2)}KB)`);
            ...
            console.error(`❌ [MediaVPS] UAZapi video download failed:`, error.message);
            throw error;
        }
    }

    /**
     * Baixa documento via UAZapi (descriptografa automaticamente)
     * 
     * @param {string} messageId - ID da mensagem
     * @param {string} token - Token da instância UAZapi
     * @param {object} metadata - Metadados (fileName, mimetype)
     * @returns {Promise<object>} { localPath, fullPath, mimeType, size, fileName }
     */
    static async downloadDocumentFromUAZapi(messageId, token, metadata = {}) {
        try {
            console.log(`📄 [MediaVPS] Downloading document via UAZapi (decryption)...`);
            ...
            // Corrigir typo comum (htps → https)
            let docUrl = data.fileURL.replace(/^htps:\/\//, 'https://');

            console.log(`📥 [MediaVPS] Downloading decrypted document from: ${docUrl}`);

            // Baixar o documento descriptografado
            const docResponse = await fetch(docUrl);

            if (!docResponse.ok) {
                throw new Error(`Failed to download document from ${docUrl}: ${docResponse.status}`);
            }

            // Detectar extensão
            const contentType = docResponse.headers.get('content-type') || 'application/pdf';
            let extension = this.getExtensionFromMimeType(contentType);
            if (!extension && metadata.mimetype) {
                extension = this.getExtensionFromMimeType(metadata.mimetype);
            }
            if (!extension) extension = 'pdf'; // Default

            // Nome do arquivo: SEMPRE usar ID gerado para segurança
            const fileName = this.generateFileName(messageId, extension);

            // Log para debug
            if (metadata.fileName) {
                console.log(`ℹ️ [MediaVPS] Original filename: ${metadata.fileName} -> Saved as: ${fileName}`);
            }

            const fullPath = path.join(process.cwd(), 'public', 'media', 'document', fileName);
            const localPath = `/media/document/${fileName}`;
            ...
            // Salvar arquivo
            const fileStream = createWriteStream(fullPath);
            await pipeline(docResponse.body, fileStream);

            const stats = fs.statSync(fullPath);

            // VALIDAR: Documento deve ter pelo menos 100 bytes
            if (stats.size < 100) {
                fs.unlinkSync(fullPath);
                throw new Error(`Downloaded document is too small (${stats.size} bytes) - likely corrupted`);
            }

            console.log(`✅ [MediaVPS] Document saved (decrypted): ${localPath} (${(stats.size / 1024).toFixed(2)}KB)`);
            ...
            console.error(`❌ [MediaVPS] UAZapi document download failed:`, error.message);
            throw error;
        }
    }

    /**
     * Sanitiza o nome do arquivo para evitar problemas no sistema de arquivos e URLs
    ...
    static sanitizeFileName(fileName) {
        ...
        // Manter apenas letras, números, pontos, traços e underscores
        sanitized = sanitized.replace(/[^a-zA-Z0-9._-]/g, '');

        // Limitar tamanho do nome (sem extensão) para evitar problemas de URL/FS
        const ext = path.extname(sanitized);
        const name = path.basename(sanitized, ext);

        // Limite seguro: 50 caracteres para o nome + extensão
        if (name.length > 50) {
    ...
    static async downloadAndSave(mediaUrl, mediaType, messageId, metadata = {}) {
        try {
            ...
            // 4. Gerar nome e caminho do arquivo
            // SEMPRE usar ID gerado para garantir segurança e compatibilidade do filesystem
            let fileName = this.generateFileName(messageId, extension);
    ...
    static async processMedia(mediaUrl, mediaType, messageId, metadata = {}) {
        try {
            ...
            // VÍDEO: Usar endpoint UAZapi para descriptografar (.enc)
            else if (mediaType === 'video' && metadata.instanceToken) {
                try {
                    fileInfo = await this.downloadVideoFromUAZapi(messageId, metadata.instanceToken);
                } catch (uazapiError) {
                    console.warn(`⚠️ [MediaVPS] UAZapi video decryption failed, using direct download:`, uazapiError.message);
                    // Fallback: download direto
                    fileInfo = await this.downloadAndSave(mediaUrl, mediaType, messageId, metadata);
                }
            }
            // DOCUMENTO: Usar endpoint UAZapi para descriptografar (.enc)
            else if (mediaType === 'document' && metadata.instanceToken) {
                try {
                    fileInfo = await this.downloadDocumentFromUAZapi(messageId, metadata.instanceToken, metadata);
                } catch (uazapiError) {
                    console.warn(`⚠️ [MediaVPS] UAZapi document decryption failed, using direct download:`, uazapiError.message);
                    // Fallback: download direto
                    fileInfo = await this.downloadAndSave(mediaUrl, mediaType, messageId, metadata);
                }
            }
            ...
            // IMPORTANTE: Construir URL pública que funciona em runtime (via rota API)
            // Next.js não serve arquivos criados após o build, então usamos /api/media/
            const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || process.env.NEXT_PUBLIC_APP_URL || 'https://swiftbot.com.br';

            // Converter /media/type/filename para /api/media/type/filename
            const apiMediaPath = fileInfo.localPath.replace(/^\/media\//, '/api/media/');
            const fullPublicUrl = `${baseUrl}${apiMediaPath}`;

            const result = {
                ...fileInfo,
                fullPublicUrl,                    // URL pública via API route (funciona em runtime)
                transcription: null,
            ...
            try {
                // Usar a URL pública já calculada (via API route que funciona em runtime)
                const publicUrl = fullPublicUrl;
                ...
                switch (mediaType) {
                    ...
                    case 'image': {
                        ...
                        // NEW behavior: Treat image interpretation as transcription
                        result.transcription = imageResult.interpretation;
                        result.transcriptionStatus = 'completed';

                        console.log(`✅ [MediaVPS] Image analyzed and transcribed`);
                        break;
                    }
                    ...
                    case 'document': {
                        ...
                        // OpenAI Vision only supports images. Check mimeType.
                        const supportedImageTypes = ['image/jpeg', 'image/jpg', 'image/png', 'image/webp', 'image/gif'];

                        if (supportedImageTypes.includes(fileInfo.mimeType)) {
                            ...
                            console.log(`✅ [MediaVPS] Document analyzed (image-based)`);
                        } else {
                            console.log(`ℹ️ [MediaVPS] Skipping OpenAI analysis for non-image document: ${fileInfo.mimeType}`);
                            result.aiInterpretation = 'Documento salvo (análise visual não disponível para este formato)';
                        }
                        break;
                    }
