...
export function createChatSupabaseClient() {
  if (!chatSupabaseUrl || !chatSupabaseAnonKey) {
    console.error('❌ [ChatDB] Variáveis de ambiente não configuradas:');
    console.error('   NEXT_PUBLIC_CHAT_SUPABASE_URL:', chatSupabaseUrl ? '✓ configurada' : '✗ faltando');
    console.error('   NEXT_PUBLIC_CHAT_SUPABASE_ANON_KEY:', chatSupabaseAnonKey ? '✓ configurada' : '✗ faltando');
    throw new Error(
      'Chat Supabase credentials not configured. ' +
      'Por favor, configure NEXT_PUBLIC_CHAT_SUPABASE_URL e NEXT_PUBLIC_CHAT_SUPABASE_ANON_KEY ' +
