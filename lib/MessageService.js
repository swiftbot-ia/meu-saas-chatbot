...
class MessageService {
  ...
  static async listMessages(conversationId, userId, { limit = 50, before = null } = {}) {
    ...
    // Get the contact's WhatsApp number
    ...
    if (reciprocalConnection) {
      // Get the current connection's phone number
      ...
      if (currentConnection) {
        // Find the contact that represents ME from the OTHER connection's perspective
    ...
    // Query messages from all relevant contact_ids
