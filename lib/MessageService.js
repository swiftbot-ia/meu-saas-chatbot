...
class MessageService {
  ...
  /**
   * Process media: download and transcribe if needed
   * @param {string} mediaUrl - URL to download media from
   * @param {string} mediaType - Type of media (audio, image, video, document)
   * @param {string} messageId - Unique message identifier
   * @param {string} mimeType - MIME type from UAZAPI payload (optional)
   * @param {string} mediaKey - WhatsApp media encryption key (base64) for encrypted media (optional)
   * @param {string} fileSHA256 - Expected SHA256 hash for verification (optional)
   */
  static async processMedia(mediaUrl, mediaType, messageId, mimeType = null, mediaKey = null, fileSHA256 = null) {
    try {
      // Initialize media storage if not done yet
      await mediaService.initializeStorage();

      // Download and save media locally (with decryption if needed)
      const mediaInfo = await mediaService.downloadAndSaveMedia(
        mediaUrl,
        mediaType,
        messageId,
        mimeType,
      ...
      // Transcribe or interpret media using OpenAI
      if (openAIService.isConfigured()) {
        try {
          console.log(`🤖 Processing ${mediaType} with OpenAI...`);

          const result = await openAIService.interpretMedia(
            mediaType,
            mediaInfo.publicUrl,
            mediaInfo.localPath
          );
          ...
          console.log(`✅ OpenAI processing completed for ${mediaType}`);

        } catch (aiError) {
          console.error('❌ Error processing with OpenAI:', aiError);
          transcriptionStatus = 'failed';
          aiInterpretation = `Erro ao processar: ${aiError.message}`;
        }
      } else {
      ...
      // The message should still be saved even if media processing fails
      return {
        localPath: null,
        publicUrl: null,
        mimeType: null,
        size: null,
        transcription: null,
        aiInterpretation: `Erro ao processar mídia: ${error.message}`,
        transcriptionStatus: 'failed',
        downloadedAt: null,
        transcribedAt: null
      };
  ...
  static async processIncomingMessage(messageData, instanceName, connectionId, userId) {
    ...
    try {
      ...
      if (messageInfo.conversation) {
        ...
        mediaMimeType = messageInfo.imageMessage.mimetype;
        mediaKey = messageInfo.imageMessage.mediaKey;
        fileSHA256 = messageInfo.imageMessage.fileSHA256;
      } else if (messageInfo.videoMessage) {
        messageType = 'video';
        messageContent = messageInfo.videoMessage.caption || '';
        mediaUrl = messageInfo.videoMessage.url;
        mediaMimeType = messageInfo.videoMessage.mimetype;
        mediaKey = messageInfo.videoMessage.mediaKey;
        fileSHA256 = messageInfo.videoMessage.fileSHA256;
      } else if (messageInfo.audioMessage) {
        messageType = 'audio';
        mediaUrl = messageInfo.audioMessage.url;
        mediaMimeType = messageInfo.audioMessage.mimetype;
        mediaKey = messageInfo.audioMessage.mediaKey;
        fileSHA256 = messageInfo.audioMessage.fileSHA256;
        ...
        mediaKey = messageInfo.documentMessage.mediaKey;
        fileSHA256 = messageInfo.documentMessage.fileSHA256;
      }

      // Process media if present (download + decrypt + transcribe)
      let mediaProcessing = null;
      if (mediaUrl) {
        console.log(`📥 Processing ${messageType} media from UAZAPI... (MIME: ${mediaMimeType || 'unknown'}, encrypted: ${!!mediaKey})`);
        mediaProcessing = await this.processMedia(
          mediaUrl,
          messageType,
          messageKey.id,
          mediaMimeType,
        ...
        // Update message content with transcription if available
        if (mediaProcessing.transcription) {
          messageContent = mediaProcessing.transcription;
        } else if (mediaProcessing.aiInterpretation && !messageContent) {
          messageContent = mediaProcessing.aiInterpretation;
        }
      }
      ...
      console.log('💾 Salvando mensagem no banco:', {
        ...
        direction: 'inbound',
        has_media: !!mediaUrl,
        media_downloaded: !!mediaProcessing?.localPath,
        has_transcription: !!mediaProcessing?.transcription
      });

      const messageData_insert = {
        instance_name: instanceName,
      ...
      // Add media processing fields if media was processed
      if (mediaProcessing) {
        messageData_insert.local_media_path = mediaProcessing.localPath;
        messageData_insert.media_mime_type = mediaProcessing.mimeType;
        messageData_insert.media_size = mediaProcessing.size;
        messageData_insert.transcription = mediaProcessing.transcription;
        messageData_insert.transcription_status = mediaProcessing.transcriptionStatus;
        messageData_insert.ai_interpretation = mediaProcessing.aiInterpretation;
        messageData_insert.media_downloaded_at = mediaProcessing.downloadedAt;
        messageData_insert.transcribed_at = mediaProcessing.transcribedAt;
      }

      const { data: message, error: msgError } = await chatSupabase
        .from('whatsapp_messages')
        .insert(messageData_insert)
        .select()
        .single();
