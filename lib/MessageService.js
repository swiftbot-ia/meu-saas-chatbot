  ...
  static async processIncomingMessage(messageData, instanceName, connectionId, userId, customChatClient = null) {
    ...
    try {
      ...
      console.log('📇 Criando/buscando contato:', {
        ...
        jid: remoteJid,
        profilePicUrl: messageData.profilePicUrl,
        fromMe
      });

      // Create or get contact in chat DB
      // IMPORTANT: For messages from me (fromMe=true), do NOT update the contact name
      // because pushName is the sender's name (us), not the contact's name
      // Only update name for incoming messages where pushName is the contact's name
      const contact = await ConversationService.getOrCreateContact(whatsappNumber, {
        name: fromMe ? null : (messageData.pushName || null), // Only update name if NOT from me
        jid: remoteJid,
        profilePicUrl: fromMe ? null : (messageData.profilePicUrl || null) // Only update pic if NOT from me
      }, chatSupabase);
