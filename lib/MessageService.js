...
class MessageService {
  ...
  static async listMessages(conversationId, userId, { limit = 50, before = null } = {}) {
    ...
    console.log('[MessageService] listMessages called:', { conversationId, userId, limit, before });

    // Verify user has access to this conversation
    const conversation = await ConversationService.getConversation(conversationId, userId);

    if (!conversation) {
      console.error('[MessageService] Conversation not found or access denied:', conversationId);
      throw new Error('Conversation not found or access denied');
    }

    console.log('[MessageService] Conversation verified:', conversation.id);
    ...
    console.log('[MessageService] Found messages:', data?.length || 0);
