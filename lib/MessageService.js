...
class MessageService {
   ...
   */
  static async processMedia(mediaUrl, mediaType, messageId, messageInfo = null) {
    try {
      // Initialize media storage if not done yet
      await mediaService.initializeStorage();

      // Extract decryption keys if available (for encrypted WhatsApp media)
      let decryptionKeys = null;
      if (messageInfo) {
        const mediaKey = messageInfo.mediaKey;
        const fileEncSha256 = messageInfo.fileEncSha256;
        const fileSha256 = messageInfo.fileSha256;
        const mimetype = messageInfo.mimetype;

        if (mediaKey && fileEncSha256 && fileSha256) {
          decryptionKeys = {
            mediaKey,
            fileEncSha256,
            fileSha256,
      ...
      const mediaInfo_result = await mediaService.downloadAndSaveMedia(
        mediaUrl,
        mediaType,
        messageId,
        decryptionKeys
      ...
      if (openAIService.isConfigured()) {
        try {
          ...
          const result = await openAIService.interpretMedia(
            mediaType,
            mediaInfo_result.publicUrl,
            mediaInfo_result.localPath
          );
      ...
      return {
        localPath: mediaInfo_result.localPath,
        publicUrl: mediaInfo_result.publicUrl,
        mimeType: mediaInfo_result.mimeType,
        size: mediaInfo_result.size,
        transcription,
        aiInterpretation,
        transcriptionStatus,
  ...
  static async processIncomingMessage(messageData, instanceName, connectionId, userId) {
    ...
    try {
      ...
      // Process media if present (download + transcribe)
      let mediaProcessing = null;
      if (mediaUrl) {
        console.log(`📥 Processing ${messageType} media from UAZAPI...`);

        // Extract media info for decryption
        let mediaInfoForDecryption = null;
        if (messageInfo.imageMessage) {
          mediaInfoForDecryption = messageInfo.imageMessage;
        } else if (messageInfo.videoMessage) {
          mediaInfoForDecryption = messageInfo.videoMessage;
        } else if (messageInfo.audioMessage) {
          mediaInfoForDecryption = messageInfo.audioMessage;
        } else if (messageInfo.documentMessage) {
          mediaInfoForDecryption = messageInfo.documentMessage;
        }

        mediaProcessing = await this.processMedia(mediaUrl, messageType, messageKey.id, mediaInfoForDecryption);
