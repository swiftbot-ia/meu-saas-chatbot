...
async function getRecentMessagesContext(connectionId, limit = 300) {
    try {
        // NOTA: Colunas da tabela whatsapp_messages:
        // - message_content (não 'content')
        // - message_type (não 'type')
        // - direction ('outbound' ou 'inbound', não 'is_from_me')
        const { data: messages, error } = await chatSupabaseAdmin
            .from('whatsapp_messages')
            .select('message_content, direction, received_at')
            .eq('connection_id', connectionId)
        ...
        if (error) {
            console.error('[ContextBuilder] Error querying messages:', error)
            return null
        }

        if (!messages?.length) {
            console.log('[ContextBuilder] No messages found for connection:', connectionId)
            return null
        }
        ...
        recentMessages.forEach(msg => {
            ...
            const content = msg.message_content?.substring(0, 200) || ''
            if (content.trim()) {
                context += `[${sender}]: ${content}\n`
            }
        })

        console.log('[ContextBuilder] Built message context with', messages.length, 'messages')
        return context
