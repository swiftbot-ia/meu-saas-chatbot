...
export const STRIPE_PRICE_IDS = {
  monthly: {
    1: 'price_1Sr4M42Y59Hv79eSihryldqD',
    2: 'price_1Sr4M42Y59Hv79eSH90t5QAN',
    3: 'price_1Sr4M52Y59Hv79eSpq8zqFFe',
    4: 'price_1Sr4M62Y59Hv79eSlPP7E99S',
    5: 'price_1Sr4M72Y59Hv79eS8qKYq5j7',
    6: 'price_1Sr4M72Y59Hv79eSresz5Tug',
    7: 'price_1Sr4M82Y59Hv79eSYxqetMTS',
  },
  annual: {
    1: 'price_1Sr4M92Y59Hv79eSsJGVTaNe',
    2: 'price_1Sr4MA2Y59Hv79eSW4GUfXTD',
    3: 'price_1Sr4MA2Y59Hv79eSQbUSOtOi',
    4: 'price_1Sr4MC2Y59Hv79eSPuaoUeiy',
    5: 'price_1Sr4MD2Y59Hv79eStEODvW9R',
    6: 'price_1Sr4MD2Y59Hv79eS0VuspbEq',
    7: 'price_1Sr4ME2Y59Hv79eSOBkbxmNc',
  }
}
 ...
 */
export async function processUpgrade(stripeSubscriptionId, newPlan, trialEnd = null) {
  try {
    ...
    console.log('📡 Buscando subscription na Stripe...')
    const subscription = await stripe.subscriptions.retrieve(stripeSubscriptionId)

    console.log('🔍 DEBUG - Subscription completa:', JSON.stringify(subscription, null, 2))
    console.log('🔍 DEBUG - current_period_end RAW:', subscription.current_period_end)
    console.log('🔍 DEBUG - current_period_end TYPE:', typeof subscription.current_period_end)
    ...
    if (subscription.current_period_end) {
      if (typeof subscription.current_period_end === 'number') {
        // É timestamp Unix (segundos) - converter para milissegundos
        periodEndDate = new Date(subscription.current_period_end * 1000)
      } else if (typeof subscription.current_period_end === 'string') {
        // É string ISO - converter direto
        periodEndDate = new Date(subscription.current_period_end)
      }

      console.log('🔍 DEBUG - periodEndDate criada:', periodEndDate)
      console.log('🔍 DEBUG - periodEndDate válida?', !isNaN(periodEndDate.getTime()))

      if (isNaN(periodEndDate.getTime())) {
        console.error('❌ Data inválida detectada!')
        throw new Error('Data de renovação inválida na subscription')
      }
    }
    ...
    console.log('✅ Subscription encontrada:', {
      ...
      current_period_end_formatted: periodEndDate ? periodEndDate.toISOString() : 'N/A'
    })
    ...
    const updateParams = {
      ...
      payment_behavior: 'error_if_incomplete', // 🚨 IMPORTANTE: Falha se o cartão não passar na hora
      metadata: {
    ...
    // Se trialEnd for fornecido, adicionamos ao update
    if (trialEnd) {
      updateParams.trial_end = trialEnd;
      // Se estamos em trial, proration deve ser 'none' para evitar invoices zeradas complexas ou cobranças indevidas,
      // mas 'create_prorations' geralmente é safe se o valor for 0.
      // Melhor manter o padrão da stripe para trial: trial_end dita quando a cobrança ocorre.
    }
    ...
    if (updatedSubscription.latest_invoice) {
      try {
        ...
        chargedAmount = invoice.amount_due / 100

        console.log('💳 Invoice gerada:', {
          id: invoice.id,
          amount_due: chargedAmount,
          status: invoice.status
        })
      } catch (invoiceError) {
        console.warn('⚠️ Erro ao buscar invoice:', invoiceError.message)
      }
    }

    // 5. Converter data de renovação de forma segura
    ...
    if (updatedSubscription.current_period_end) {
      ...
      } else if (typeof updatedSubscription.current_period_end === 'string') {
        const tempDate = new Date(updatedSubscription.current_period_end)
        nextBillingDate = !isNaN(tempDate.getTime()) ? tempDate.toISOString() : null
      }
    }
