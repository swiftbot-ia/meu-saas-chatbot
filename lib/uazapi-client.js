...
class UAZAPIClient {
   ...
   * Endpoint: POST /message/download (documentação oficial)
   * Auth: token (da instância)
   *
   * WhatsApp envia URLs de mídia criptografadas. A UazAPI descriptografa
   * e retorna o arquivo original.
   *
   * IMPORTANTE: Para áudios, usa generate_mp3: true para retornar MP3 ao invés de OGG
   * Isso elimina necessidade de conversão FFmpeg e garante compatibilidade com Whisper
   *
   * @param {string} instanceToken - Token da instância
   * @param {string} messageId - ID da mensagem (não URL!)
   * @param {string} messageType - Tipo da mensagem (audio, image, video, document)
   * @returns {Promise<Buffer>} Buffer do arquivo descriptografado
   */
  async downloadMedia(instanceToken, messageId, messageType = null) {
    ...
    // Para áudios, solicitar MP3 já convertido (evita FFmpeg)
    const body = {
      id: messageId
    }
    ...
    if (messageType === 'audio') {
      ...
      console.log('🎵 Solicitando áudio como MP3 (generate_mp3: true)')
    }

    try {
      console.log(`📡 UAZAPI Download: messageId=${messageId.substring(0, 20)}...`)
      ...
      // A UazAPI retorna um JSON com fileURL, não o arquivo direto
      const jsonResponse = await response.json()

      if (!jsonResponse.fileURL) {
        throw new UAZAPIError('UazAPI não retornou fileURL', 500, jsonResponse)
      }

      console.log(`📥 Baixando arquivo de: ${jsonResponse.fileURL.substring(0, 60)}...`)

      // Baixar o arquivo real da URL retornada
      const fileResponse = await fetch(jsonResponse.fileURL)

      if (!fileResponse.ok) {
        throw new UAZAPIError(
          `Erro ao baixar arquivo da UazAPI: ${fileResponse.statusText}`,
          fileResponse.status,
          null
        )
      }
      ...
      const fileType = messageType === 'audio' && body.generate_mp3 ? 'MP3' : 'original'
      console.log(`✅ UAZAPI Download Success: ${buffer.byteLength} bytes (${fileType})`)
