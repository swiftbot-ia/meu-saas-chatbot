...
class UAZAPIClient {
  ...
  async sendAudio(instanceToken, to, audioUrl, ptt = false) {
    ...
    // Some versions of the API might still look for ptt: true, so keeping it doesn't hurt
  ...
  /**
   * Obtém detalhes de um contato (nome, foto, etc)
   * Endpoint: POST /chat/details
   * Auth: token (da instância)
   * 
   * @param {string} instanceToken - Token da instância
   * @param {string} number - Número do contato
   * @returns {Promise<object>} Detalhes do contato
   */
  async getContactDetails(instanceToken, number) {
    return this.request('/chat/details', {
      method: 'POST',
      body: JSON.stringify({
        number,
        preview: true
   ...
   */
  async getContacts(instanceToken) {
   ...
   */
  async getMessages(instanceToken, chatId) {
    const response = await this.request('/message/find', {
      method: 'POST',
      body: JSON.stringify({
        chatid: chatId
        // Sem limite - pegar todas as mensagens
      })
    }, 'instance', instanceToken)

    // Debug: log response structure
    if (!this._messageStructureLogged) {
      console.log('🔍 [UAZAPI] Resposta /message/find:', {
        type: typeof response,
        isArray: Array.isArray(response),
        keys: response ? Object.keys(response) : null,
        length: Array.isArray(response) ? response.length : (response?.messages?.length || response?.data?.length || 'N/A'),
        sample: JSON.stringify(response).substring(0, 800)
      })
      this._messageStructureLogged = true
    }

    // A API pode retornar formatos diferentes
   ...
   * @param {boolean} returnBase64 - Retornar mídia em base64
   * @param {boolean} generateMp3 - Converter áudio para MP3
   * @returns {Promise<object>} Dados da mídia
   */
  async downloadMedia(instanceToken, messageId, returnBase64 = true, generateMp3 = true) {
