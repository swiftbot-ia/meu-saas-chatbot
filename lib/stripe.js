...
// ============================================================================
// CRIAR PAYMENT METHOD (substitui createCard do Pagar.me)
// ============================================================================
export async function createPaymentMethod(cardData) {
  try {
    console.log('Criando Payment Method na Stripe...')

    // Stripe usa Payment Methods ao invés de Cards diretos
    const paymentMethodData = {
      type: 'card',
      'card[number]': cardData.card_number.replace(/\s/g, ''),
      'card[exp_month]': cardData.card_expiration_month,
      'card[exp_year]': cardData.card_expiration_year,
      'card[cvc]': cardData.card_cvv
    }

    const paymentMethod = await stripeRequest('/payment_methods', 'POST', paymentMethodData)
