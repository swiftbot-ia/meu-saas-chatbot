  ...
  try {
    ...
    if (method !== 'GET' && method !== 'DELETE') {
      if (Array.isArray(data)) {
        // Se for array de entradas (entries), passamos direto para URLSearchParams
        options.body = new URLSearchParams(data).toString()
      } else {
        // Se for objeto, convertemos
        options.body = new URLSearchParams(data).toString()
      }
    }
...
export async function createSubscription({
  ...
  promotionCode = null // <--- Renamed from coupon to strictly match usage
}) {
  try {
    ...
    const subscriptionData = [
      ...
      ['expand[]', 'latest_invoice'],                 // <--- GARANTIR QUE INVOICE VENHA
      ['expand[]', 'latest_invoice.payment_intent'], // <--- GARANTIR QUE INTENT VENHA DENTRO DA INVOICE
    ...
    // ✅ Adicionar Promotion Code (Customer Facing Code)
    if (promotionCode) {
      console.log(`🎟️ Aplicando Promotion Code: ${promotionCode}`)
      subscriptionData.push(['promotion_code', promotionCode])
    }
