  ...
  useEffect(() => {
    const checkAffiliateStatus = async () => {
      try {
        ...
        if (data.success && data.is_affiliate) {
          setIsAffiliate(true)
        }
      } catch (error) {
            ...
            <div className="px-3 mb-6">
              <Link
                ...
                className={`
                    ...
                    bg-[#00FF99] text-black hover:bg-[#00E88C] hover:scale-105 hover:shadow-[0_0_20px_rgba(0,255,153,0.3)]
                    ${!(expanded || isHovering) && 'justify-center'}
                  `}
              >
                <Zap size={20} className="flex-shrink-0" fill="currentColor" />

                <span className={`flex-1 text-sm font-bold whitespace-nowrap overflow-hidden transition-all duration-300 ${(expanded || isHovering) ? 'opacity-100 max-w-full' : 'opacity-0 max-w-0'}`}>
                  {isAffiliate ? 'Portal de Afiliados' : 'Torne-se um Parceiro'}
                </span>
              </Link>
            </div>
          )}
